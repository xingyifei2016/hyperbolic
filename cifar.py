...
## Code adapted from https://github.com/sairin1202/BIC
## Credits to sairin1202

class Cifar100:
    def __init__(self, path='./cifar100'):
        self.path = path
        
        if not os.path.exists(self.path):
            # Extract file
            urllib.request.urlretrieve("https://www.cs.toronto.edu/~kriz/cifar-100-python.tar.gz", filename="./cifar-100-python.tar.gz")
            file = tarfile.open('./cifar-100-python.tar.gz')
            file.extractall(self.path)
            file.close()
            shutil.move(self.path+"/cifar-100-python/meta", self.path+"/meta")
            shutil.move(self.path+"/cifar-100-python/train", self.path+"/meta")
            shutil.move(self.path+"/cifar-100-python/test", self.path+"/meta")

        with open(self.path+'/train','rb') as f:
            self.train = pickle.load(f, encoding='latin1')
        with open(self.path+'/test','rb') as f:
    ...
    def initialize(self, num_groups=20):
        # Initialize the dataset to run over **num_groups** number of groups
        # CIFAR100 has 100 classes --> we can do 20 groups, 5 class per group
        assert(
        train_groups = [[],[],[],[],[]]
